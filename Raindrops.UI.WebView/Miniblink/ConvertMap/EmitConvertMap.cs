...
public static class EmitConvertMap
{
    ...
    public static Type GetEmumType(Type type)
    {
        switch (Type.GetTypeCode(type))
        {
            case TypeCode.Byte:
    ...
    private static void Init()
    {
        ...
        for (int i = 0; i < s_numbers.Length; i++)
        {
            ...
            //Number to String
            dic.TryAdd(typeof(string), new ConvertItem((il) =>
            {
                il.BoxIfNeeded(source);
                il.Call(cucMethodInfo);
                il.Call(typeof(IConvertible).GetMethod(nameof(IConvertible.ToString)));
            }));
            //Number to Boolen
            dic.TryAdd(typeof(bool), new ConvertItem((il) =>
            {
                il.BoxIfNeeded(source);
                il.Call(cucMethodInfo);
                il.Call(typeof(IConvertible).GetMethod(nameof(IConvertible.ToBoolean)));
            }, sSize - 1, 5, s_numbers.Length));
    ...
    {
        if (source.IsEnum) source = GetEmumType(source);
        if (target.IsEnum) target = GetEmumType(target);
        convertItem = default;
